...
$q_total_bulan = $conn->query("
    SELECT
        ...
        SUM(CASE WHEN status = 'pemasukan' THEN biaya_spp ELSE 0 END) as total_pemasukan,
        SUM(CASE WHEN status = 'pengeluaran' THEN biaya_spp ELSE 0 END) as total_pengeluaran,
        (SUM(CASE WHEN status = 'pemasukan' THEN biaya_spp ELSE 0 END) -
         SUM(CASE WHEN status = 'pengeluaran' THEN biaya_spp ELSE 0 END)) as saldo
    FROM spp
...
$q_total_keseluruhan = $conn->query("
    SELECT
        SUM(CASE WHEN status = 'pemasukan' THEN biaya_spp ELSE 0 END) as total_pemasukan_all,
        SUM(CASE WHEN status = 'pengeluaran' THEN biaya_spp ELSE 0 END) as total_pengeluaran_all,
        (SUM(CASE WHEN status = 'pemasukan' THEN biaya_spp ELSE 0 END) -
         SUM(CASE WHEN status = 'pengeluaran' THEN biaya_spp ELSE 0 END)) as saldo_all
    FROM spp
");
...
$q_bulan_ini = $conn->query("
    SELECT
        SUM(CASE WHEN status = 'pemasukan' THEN biaya_spp ELSE 0 END) as pemasukan_bulan_ini,
        SUM(CASE WHEN status = 'pengeluaran' THEN biaya_spp ELSE 0 END) as pengeluaran_bulan_ini,
        (SUM(CASE WHEN status = 'pemasukan' THEN biaya_spp ELSE 0 END) -
         SUM(CASE WHEN status = 'pengeluaran' THEN biaya_spp ELSE 0 END)) as saldo_bulan_ini
    FROM spp
    WHERE DATE_FORMAT(date, '%Y-%m') = '$bulan_ini'
");
...
<div class="container-fluid py-4">
  ...
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 text-primary">
      <i class="bi bi-graph-up"></i> Laporan Keuangan SPP
    </h1>
    <div class="btn-group">
              ...
              <tbody>
                <?php while($row = $q_pemasukan->fetch_assoc()): ?>
                  <tr>
                    ...
                    <td class="text-end">Rp <?= number_format($row['biaya_spp']) ?></td>
                    <td><span class="badge bg-success">pemasukan</span></td>
                    <td>
                      <div class="btn-group btn-group-sm">
                        <a href="edit_spp.php?id=<?= $row['id'] ?>" class="btn btn-outline-warning">
                          <i class="bi bi-pencil"></i>
                        </a>
                        <a href="hapus_spp.php?id=<?= $row['id'] ?>" class="btn btn-outline-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
