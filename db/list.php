...
if (isset($_GET['delete'])) {
    ...
    $delete_stmt = $conn->prepare("DELETE FROM db WHERE id=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    if ($siswa_nik && $tahun_ajaran) {
        $update_stmt = $conn->prepare("UPDATE db SET tanggal=?, tahun_ajaran=?, siswa_nik=?, nominal=?, ket=? WHERE id=?");
        $update_stmt->bind_param("sssssi", $tanggal, $tahun_ajaran, $siswa_nik, $nominal, $ket, $id);
    } else {
        $update_stmt = $conn->prepare("UPDATE db SET tanggal=?, nominal=?, ket=? WHERE id=?");
...
if (isset($_GET['export']) && $_GET['export'] == 'excel') {
    ...
    header('Content-Disposition: attachment; filename=laporan_db.csv');
    $fp = fopen('php://output', 'w');
    // Header untuk pemasukan
    fputcsv($fp, ['Pemasukan DB']);
    fputcsv($fp, ['Tanggal', 'NIK', 'Tahun Ajaran', 'Nominal', 'Keterangan']);
    $q1 = $conn->query("SELECT * FROM db WHERE siswa_nik IS NOT NULL AND siswa_nik<>'' AND tahun_ajaran IS NOT NULL AND tahun_ajaran<>'' ORDER BY tanggal DESC");
    while ($row = $q1->fetch_assoc()) {
        fputcsv($fp, [$row['tanggal'], $row['siswa_nik'], $row['tahun_ajaran'], $row['nominal'], $row['ket']]);
    }
    fputcsv($fp, []);
    // Header untuk pengeluaran
    fputcsv($fp, ['Pengeluaran DB']);
    fputcsv($fp, ['Tanggal', 'Nominal', 'Keterangan']);
    $q2 = $conn->query("SELECT * FROM db WHERE status='pengeluaran' ORDER BY tanggal DESC");
...
<head>
    ...
    <title>Daftar Pemasukan & Pengeluaran daftar baru</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
<div class="container mt-5">
    <h3>Daftar Pemasukan & Pengeluaran daftar baru</h3>
    <div class="mb-3">
    ...
    <div class="row">
        <div class="col-md-6">
            <h5 class="mt-4">Pemasukan daftar baru</h5>
        ...
        <div class="col-md-6">
            <h5 class="mt-4">Pengeluaran daftar baru</h5>
            <table class="table table-bordered table-sm">
                <thead>
                    <tr>
                        <th>ID</th>
    ...
    <div class="alert alert-info mt-4"><strong>Saldo daftar baru saat ini: <?= number_format($saldo) ?></strong></div>
</div>
<?php
