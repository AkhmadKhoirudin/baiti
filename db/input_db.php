    ...
    if (empty($tanggal) || empty($nominal) || empty($tipe)) {
        ...
        if ($tipe === "pemasukan") {
            if (empty($siswa_nik) || empty($tahun_ajaran)) {
                ...
                $insert = $conn->prepare("INSERT INTO db (siswa_nik, tahun_ajaran, tanggal, nominal, ket) VALUES (?, ?, ?, ?, ?)");
            ...
            // Pengeluaran
            $status = 'pengeluaran';
            $insert = $conn->prepare("INSERT INTO db (tanggal, nominal, ket, status) VALUES (?, ?, ?, ?)");
